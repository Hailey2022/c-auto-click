...
INT FlashConfig()
{
	...
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\AutoClick"), NULL, KEY_READ | KEY_WOW64_64KEY, &hKey) == ERROR_SUCCESS)
	{
...
INT SaveConfig()
{
	...
	if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\AutoClick"), NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS | KEY_WOW64_64KEY, NULL, &hKey, &dwDisposition) == ERROR_SUCCESS)
	{
...
LRESULT WINAPI CtlProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		hTip1 = CreateWindow(TEXT("Static"), TEXT("Mouse:"), WS_CHILD | WS_VISIBLE, 15, 10, 300, 100, hWnd, NULL, hWnd, 0);
		hRadio_Left = CreateWindow(TEXT("Button"), TEXT("left"), WS_CHILD | WS_VISIBLE | BS_LEFT | BS_AUTORADIOBUTTON, 60, 35, 80, 20, hWnd, (HMENU)ID_BtnSel, hWnd, 0);
		hRadio_Right = CreateWindow(TEXT("Button"), TEXT("right"), WS_CHILD | WS_VISIBLE | BS_LEFT | BS_AUTORADIOBUTTON, 200, 35, 80, 20, hWnd, (HMENU)ID_BtnSel, hWnd, 0);
		hTip2 = CreateWindow(TEXT("Static"), TEXT("Method:"), WS_CHILD | WS_VISIBLE, 15, 60, 300, 100, hWnd, NULL, hWnd, 0);
		...
		hTip3 = CreateWindow(TEXT("Static"), TEXT("Interval (ms):"), WS_CHILD | WS_VISIBLE, 15, 120, 300, 100, hWnd, NULL, hWnd, 0);
		hText_TL = CreateWindow(TEXT("Edit"), TEXT("500"), ES_CENTER | WS_CHILD | WS_VISIBLE, 30, 145, 80, 25, hWnd, (HMENU)ID_TLSet, hWnd, 0);
		hTip4 = CreateWindow(TEXT("Static"), TEXT("Hotkey:"), WS_CHILD | WS_VISIBLE, 210, 120, 300, 100, hWnd, NULL, hWnd, 0);
		...
		hBtn_UorE = CreateWindow(TEXT("Button"), TEXT("Start"), ES_CENTER | WS_CHILD | WS_VISIBLE, 25, 180, 290, 50, hWnd, (HMENU)ID_UorELock, hWnd, 0);
		...
		SendMessage(hCombo_Func, CB_ADDSTRING, 0, TEXT("mouse_event"));
		SendMessage(hCombo_Func, CB_ADDSTRING, 0, TEXT("SendInput"));
	...
	case WM_COMMAND:
		...
		case ID_UorELock:
			if (CfgLocked)
			{
				...
				EnableWindow(hRadio_Left, TRUE);
				SendMessage(hBtn_UorE, WM_SETTEXT, 0, TEXT("Save"));
			}
			else
			{
				...
				SendMessage(hBtn_UorE, WM_SETTEXT, 0, TEXT("Stop"));
				CfgLocked = TRUE;
				SaveConfig();
			}
			break;
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	RegisterClassEx(&WC);
	hwnd = CreateWindow(TEXT("WND"), TEXT("Auto Click"), WS_OVERLAPPEDWINDOW ^ WS_THICKFRAME ^ WS_MAXIMIZEBOX, width / 2 - 178, height / 2 - 155, 356, 280, NULL, 0, 0, 0);
	ShowWindow(hwnd, 1);
	UpdateWindow(hwnd);
